...
export class RecaudacionesComponent implements OnInit {
 ...
 // muestraResumenbool: boolean = false;
  ...
  constructor(private reportService: ReportService) {
    this.TIPO_DOCUMENTO = "RECAUDACIONES"; 
   }
...
///Para generar el reporte en excel del detalle de Recaudaciones
  public getReporteRec(tipoReporte: string) {
    let params = new HttpParams()
      .set('PERIODO', 'ST' + this.periodoContable)
      .set('CLASE', 'ST' +  this.claseDocumento)
      let name: string ="MCWDetalleRecaudaciones";
    if(tipoReporte == "MCWDetalleRecaudaciones"){
      name = 'MCWDetalleRecaudaciones';
    }
    let type = 'XLSX';
    //console.log(params);
    //console.log(name);
